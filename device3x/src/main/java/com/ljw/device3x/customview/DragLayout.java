...
public class DragLayout extends ViewGroup {
	...
	private ShowPreviousPageNotifier previousPageNotifier;//手指松开是否加载上一页的notifier
...
//			Log.i("ljwtest:", "viewHeight" + viewHeight);
			int offsetTopBottom = frameView2.getTop() - viewHeight
					- frameView1.getTop();
			frameView1.offsetTopAndBottom(offsetTopBottom);
		} else if(viewIndex == 3) {
			frameView2.offsetTopAndBottom(-400);
		} else if(viewIndex == 4) {
			Log.i("ljwtest:", "向上滑了");
			frameView1.offsetTopAndBottom(400);
		}
	...
	private void animTopOrBottom(View releasedChild, float yvel) {
		...
		if (releasedChild == frameView1) {
			...
			if (yvel > VEL_THRESHOLD
				...
				// 上一页可以加载了
				if(null != previousPageNotifier) {
					previousPageNotifier.onDragPrevious();
				}
			}
		...
		if (mDragHelper.smoothSlideViewTo(releasedChild, 0, finalTop)) {
			Log.i("ljwtest:", "执行到了smoothSlideViewTo");
			ViewCompat.postInvalidateOnAnimation(this);
		}
	}
	...
	public void setPreviousListener(ShowPreviousPageNotifier previousPageListener) {
		this.previousPageNotifier = previousPageListener;
	}
	...
	public interface  ShowPreviousPageNotifier {
		public void onDragPrevious();
	}

	public void jumpToSecondPage() {
		onViewPosChanged(3, 1);
	}
}
